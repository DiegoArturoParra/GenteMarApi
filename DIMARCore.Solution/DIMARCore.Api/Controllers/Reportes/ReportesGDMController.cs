...
namespace DIMARCore.Api.Controllers.Reportes
{
    ...
    public class ReportesGDMController : BaseApiController
    {
        ...
        /// <response code="404">NotFound. No se ha encontrado ningun dato del filtro.</response>
        /// <response code="500">Internal Server Error. ha ocurrido un error.</response>
        /// <returns></returns>
        [ResponseType(typeof(ResponseTypeSwagger<FileTocsvDTO>))]
        [HttpPost]
        [Route("generar-csv-datos-basicos")]
        public async Task<IHttpActionResult> GenerarCSVDatosBasicos([FromBody] DatosBasicosReportFilter reportFilter)
        {
            var report = await _reportesGDMBusiness.GenerateReportDatosBasicosCSV(reportFilter);
            return Ok(report);
        }
    }
}
