...
namespace DIMARCore.Repositories.Repo
{
    public class ReglaCargoRepository : GenericRepository<GENTEMAR_REGLAS_CARGO>
    {
        ...
        public async Task<DetalleCargoReglaDTO> GetDetalleById(int reglaCargoId)
        {
            ...
            var detalle = await query.Select(m => new DetalleCargoReglaDTO
            {
        ...
        public async Task<int> GetIdReglaCargo(IdsLlaveCompuestaDTO idsTablas)
        {
            return await _context.GENTEMAR_REGLAS_CARGO.Where(x => x.id_regla == idsTablas.ReglaId && x.id_cargo_titulo == idsTablas.CargoTituloId)
