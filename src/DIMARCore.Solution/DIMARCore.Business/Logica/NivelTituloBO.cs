...
namespace DIMARCore.Business.Logica
{
    public class NivelTituloBO : IGenericCRUD<GENTEMAR_NIVEL, int>
    {
        ...
        {
            var niveles = await new NivelTituloRepository().GetNivelesForReglaCargo(ids);

            if (!niveles.Any())
                throw new HttpStatusCodeException(Responses.SetNotFoundResponse("No hay ninguna relación con la regla, capacidad y cargo solicitado."));
