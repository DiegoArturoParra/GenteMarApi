...
namespace DIMARCore.UIEntities.DTOs.Reports
{
    ...
    public class DataPieChartVciteDTO
    {
        ...
        static private string GenerateRandomColor(string estado, string isVigente)
        {
            ...
            else if (estado.Equals(EnumConfig.GetDescription(EstadoEstupefacienteEnum.Exitosa)) && isVigente.ToUpper().Contains("NO VIGENTE"))
            {
                return "#e6ec3e";
            }

            else if (estado.Equals(EnumConfig.GetDescription(EstadoEstupefacienteEnum.Exitosa)) && isVigente.ToUpper().Contains("NO CONTIENE FECHA"))
            {
                return "#7c7adc";
            }
