...
namespace DIMARCore.Api.Controllers.Estupefacientes
{
    ...
    public class EstupefacienteController : BaseApiController
    {
        ...
        [ResponseType(typeof(Paginador<ListarEstupefacientesDTO>))]
        [HttpPost]
        [Route("filtro")]
        ...
        public async Task<IHttpActionResult> PaginarAsync([FromBody] EstupefacientesFilter filtro)
        {
            ...
            var paginador = Paginador<ListarEstupefacientesDTO>.CrearPaginador(queryable.Count(), listado, filtro.Paginacion);
            return Ok(paginador);
        ...
        {
            var respuesta = await _serviceEstupefacientes.GetDatosGenteMarValidarVciteEnConsultaOVigente(obj.Identificacion);
            return Ok(respuesta);
        }

        // GET: Datos de gente de mar en datos basicos validar si existe en gente de mar y si tiene vcite vigente o en consulta
        /// <summary>
        /// Datos de gente de mar en datos basicos validar si existe en gente de mar y si tiene vcite vigente o en consulta
        /// </summary>
        /// <remarks>
        /// <Autor>Diego Parra</Autor>
        /// <Fecha>26/08/2024</Fecha>
        /// </remarks>
        /// <response code="401">Unauthorized. No se ha indicado o es incorrecto el Token JWT de acceso.</response>              
        /// <response code="200">OK. Devuelve el objeto solicitado.</response>        
        /// <response code="404">NotFound. No se ha encontrado el objeto solicitado.</response>
        ...
        [HttpGet]
        [AuthorizeRolesFilter(RolesEnum.AdministradorVCITE, RolesEnum.GestorVCITE, RolesEnum.JuridicaVCITE, RolesEnum.ConsultasVCITE)]
        [Route("datosBasicos-validar-vcite-consulta-vigente")]
        public async Task<IHttpActionResult> GetDatosBasicosExisteEnGenteDeMarValidarVcite([FromUri] CedulaDTO obj)
        {
            var respuesta = await _serviceEstupefacientes.GetDatosBasicosExisteEnGenteDeMar(obj.Identificacion);
            return Ok(respuesta);
        }
        ...
        /// <response code="500">Internal Server Error. ha ocurrido un error.</response>
        /// <returns></returns>
        [ResponseType(typeof(List<ListarEstupefacientesEdicionMasivaDTO>))]
        [HttpPost]
        ...
        [Route("crear")]
        [AuthorizeRolesFilter(RolesEnum.AdministradorVCITE, RolesEnum.GestorVCITE, RolesEnum.JuridicaVCITE)]
        public async Task<IHttpActionResult> Crear([FromBody] CrearEstupefacienteDTO estupefaciente)
        {
            var dataEstupefaciente = Mapear<CrearEstupefacienteDTO, GENTEMAR_ANTECEDENTES>(estupefaciente);
