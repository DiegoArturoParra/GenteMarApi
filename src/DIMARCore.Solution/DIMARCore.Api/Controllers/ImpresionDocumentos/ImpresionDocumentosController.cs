...
namespace DIMARCore.Api.Controllers
{
    ...
    [AuthorizeRolesFilter(RolesEnum.AdministradorGDM, RolesEnum.GestorSedeCentral, RolesEnum.Capitania)]
    public class ImpresionDocumentosController : BaseApiController
    {
        ...
        {
            var data = await _service.GetPrevistaLicencia(idLicencia, PathActual);
            return Ok(data);
        }
        /// <summary>
        ...
        {
            var data = await _service.GetPrevistaTitulo(idTitulo, PathActual);
            return Ok(data);
        }
