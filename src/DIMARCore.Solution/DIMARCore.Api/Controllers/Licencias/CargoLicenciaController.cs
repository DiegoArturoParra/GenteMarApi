...
namespace DIMARCore.Api.Controllers.Licencias
{
    ...
    public class CargoLicenciaController : BaseApiController
    {
        ...
        public async Task<IHttpActionResult> CargoLicenciaListAsync([FromBody] CargoLicenciaFilter filtro)
        {
            if (filtro == null)
            {
                filtro = new CargoLicenciaFilter
                {
                    Paginacion = new ParametrosPaginacion()
                };
            }
            var queryable = _service.GetCargosLicenciaAsync(filtro);
            var listado = await GetPaginacion(filtro.Paginacion, queryable);
            var paginador = Paginador<CargoInfoLicenciaDTO>.CrearPaginador(queryable.Count(), listado, filtro.Paginacion);
            return Ok(paginador);
