...
namespace DIMARCore.Repositories
{
    public class GenericRepository<T> : IGenericRepository<T>, IDisposable where T : class
    {
        ...
        // Constructor que inicializa el contexto desde afuera
        public GenericRepository(GenteDeMarCoreContext context)
        {
        ...
        // Constructor predeterminado que obtiene el contexto según el entorno
        public GenericRepository()
        {
            _isEnvironment = ConfigurationManager.AppSettings[Constantes.KEY_ENVIRONMENT];
            _context = GetContext(_isEnvironment);
            _entities = _context.Set<T>(); // Asegúrate de inicializar _entities
            IsDisposed = false;
        }
        ...
        private GenteDeMarCoreContext GetContext(string _isEnvironment)
        {
            ...
            if ((int)EnvironmentEnum.Production == enviroment)
            {
                ...
                var desencryptConnection = EncryptDecryptService.GenerateDecrypt(connectionString);
        ...
        public void CommitTransaction() => _context.Database.CurrentTransaction?.Commit();

        public void RollbackTransaction() => _context.Database.CurrentTransaction?.Rollback();
        ...
        #region get de un objeto con condiciones async
        public T GetWithCondition(Expression<Func<T, bool>> whereCondition) => Entities.Where(whereCondition).FirstOrDefault();
        #endregion

        #region get de un objeto con condiciones async
        public async Task<T> GetWithConditionAsync(Expression<Func<T, bool>> whereCondition)
            => await Entities.Where(whereCondition).FirstOrDefaultAsync();
        #endregion

        #region existe por condiciones async
        ...
        // Método Dispose marcado como virtual
        public virtual void Dispose()
        {
            Dispose(true);
            GC.SuppressFinalize(this);  // Evita que el finalizador sea llamado.
        }

        // Sobrecarga de Dispose que permite liberar recursos específicos.
