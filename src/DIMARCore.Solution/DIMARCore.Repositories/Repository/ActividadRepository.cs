...
namespace DIMARCore.Repositories.Repository
{
    public class ActividadRepository : GenericRepository<GENTEMAR_ACTIVIDAD>
    {
        ...
        {
            return await _context.GENTEMAR_ACTIVIDAD
                .Where(x => idsTipoLicencia.Contains(x.id_tipo_licencia) && x.activo == Constantes.ACTIVO)
                .Select(x => new ActividadLicenciaDTO
                {
                    Id = x.id_actividad,
                    IsActive = x.activo,
