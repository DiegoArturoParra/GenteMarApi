...
namespace DIMARCore.Repositories.Repository
{
    public class UsuarioRepository : GenericRepository<APLICACIONES_LOGINS>
    {
        ...
        public async Task UpdateUserRolesGDM(APLICACIONES_LOGINS user, List<int> rolesId)
        {
            var rolesActivos = await (from login in _context.APLICACIONES_LOGINS
                                      ...
                                      where login.ID_LOGIN == user.ID_LOGIN && login_rol.ID_ESTADO == (byte)EstadoUsuarioLoginEnum.ACTIVO
                                       && (roles.ID_APLICACION == (int)TipoAplicacionEnum.GenteDeMar || roles.ID_APLICACION == (int)TipoAplicacionEnum.DIM)
                                      select login_rol).ToListAsync();

            if (rolesActivos.Any())
                _context.APLICACIONES_LOGIN_ROL.RemoveRange(rolesActivos);
            ...
            if (user != null)
            {
                ...
                userSesion = await query.Select(m => new UserSesionDTO
                {
                    ...
                    Aplicacion = (from aplicaciones in _context.APLICACIONES
                                  ...
                                  select new AplicacionSessionDTO()
                                  {
                                      Id = aplicaciones.ID_APLICACION,
                                      Nombre = aplicaciones.NOMBRE
                                  }).FirstOrDefault(),
                    Capitania = (from capitania in _context.APLICACIONES_CAPITANIAS
                                 where capitania.ID_CAPITANIA == m.login.ID_CAPITANIA
                                 select new CapitaniaSessionDTO()
                                 {
            ...
            if (user != null)
            {
                ...
                userSesion = await query.Select(m => new UserSesionDTO
                {
                    ...
                    Roles = (from login_rol in _context.APLICACIONES_LOGIN_ROL
                             ...
                             where login_rol.ID_LOGIN == user.ID_LOGIN && detalleRoles.ID_APLICACION == (int)TipoAplicacionEnum.GenteDeMar
                             && login_rol.ID_ESTADO == (byte)EstadoUsuarioLoginEnum.ACTIVO
                             select new RolSessionDTO { Id = detalleRoles.ID_ROL, NombreRol = detalleRoles.ROL }).ToList()
                }).AsNoTracking().FirstOrDefaultAsync();
        ...
        public async Task<IEnumerable<InfoUsuarioDTO>> GetUsuariosConRoles()
        {
            var query = (from loginRol in _context.APLICACIONES_LOGIN_ROL
                         ...
                         where (roles.ID_APLICACION == (int)TipoAplicacionEnum.GenteDeMar || roles.ID_APLICACION == (int)TipoAplicacionEnum.DIM)
                         && roles.ID_ESTADO == (byte)EstadoUsuarioLoginEnum.ACTIVO
                         select new
                         {
                             login,
                             roles,
        ...
        public async Task<bool> InactivarOActivarUsuarioGDM(int loginId)
        {
            bool activar = true;
